...
ind_col <- which(names(train_data) == "Exited")

default_idx <- sample(nrow(train_data), nrow(train_data) * 0.7)
train <- train_data[default_idx, ]
test  <- train_data[-default_idx, ]
...
(nb_base <- naiveBayes(Exited ~ ., data = X_train))
...
for (i in 1:nrow(nb_grid)) {
  ...
  nb_model <- naiveBayes(Exited ~ ., data = X_train,
...
# 1) Crear matrices numéricas con las mismas columnas
...
# 2) PCA en TRAIN y proyección en TEST
