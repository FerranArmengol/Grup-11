...
# BAGGING con diferentes estrategias de balanceo

# Arreglar nombres ANTES de ROSE
...
tuneGrid <- data.frame(mtry = unique(pmax(1, pmin(floor(c(mtry.class/2, mtry.class, 2*mtry.class)), num_pred))))
tuneGrid
...
# Limpiar columnas
#names(train_rose) <- make.names(names(train_rose))

# Limpiar factores predictivos
#train_rose <- train_rose %>%
 # mutate(across(-Exited, ~ if(is.factor(.)) {
  #  vals <- trimws(as.character(.))  # eliminar espacios
   # vals <- make.unique(vals)         # asegura niveles únicos
    #factor(vals)
  #} else .))

# Asegurar factor respuesta
#train_rose$Exited <- factor(train_rose$Exited, levels = c(0,1), labels = c("No","Yes"))
# Revisar niveles duplicados
#sapply(train_rose, function(x) if(is.factor(x)) any(duplicated(levels(x))) else FALSE)



#set.seed(1234)
#modelo_rose3 <- caret::train(
 # Exited ~ .,
  #data = train_rose,
  #method = "rf",
  #tuneGrid = tuneGrid,
...
# Aplicar punto de corte correspondiente
pred_smote_test <- ifelse(probs_smote_test > 0.2, "Yes", "No")
...
write.csv(submission_smote, "bagging_smote_0.2.csv", row.names = FALSE)
