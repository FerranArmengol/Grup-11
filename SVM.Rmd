...
for (method in balance_methods) {
  ...
  if (method == "none") {
    ...
    train_bal <- ROSE(Exited ~ ., data = train_mod, seed = 123)$data
  }
...
svm_cv2_linear <- tune("svm", Exited ~ ., data = train, kernel = 'linear',
               ranges = list(cost = c(0.01, 0.1, 1, 5)), scale=TRUE)
...
svm_cv_poly <- tune( "svm", Exited ~ ., data = train, kernel = "polynomial", ranges = list(cost = c(0.1, 1, 10),gamma = c(0.1),degree = c(2, 3), coef0 = c(0)),scale = TRUE)
...
modelo_svm_poly <- svm(Exited ~ .,data = train,kernel = "polynomial",cost = best$cost,gamma = best$gamma,degree = best$degree,coef0 = best$coef0,  probability = TRUE, scale = TRUE)
