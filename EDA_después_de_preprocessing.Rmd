...
pvals_num <- sapply(varNum, function(v) {
  mod <- lm(datos_missing[[v]] ~ datos_missing$Exited)
  anova(mod)[["Pr(>F)"]][1]
})

# Variables categóricas 
pvals_cat <- sapply(varCat, function(v) {
  if (length(unique(na.omit(datos_missing[[v]]))) > 1) {
    suppressWarnings(chisq.test(table(datos_missing[[v]], datos_missing$Exited))$p.value)
  } else {
    NA  # variable con solo una categoría
  }
})
...
datos_relevantes <- datos_missing[, c("ID", variables_relevantes)]
