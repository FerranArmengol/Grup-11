...
dist_mahal_rob <- mahalanobis(datos_num, center = cov_rob@center, cov = cov_rob@cov)
umbral_rob <- qchisq(0.999, df = ncol(datos_num))
outliers_mahal_rob <- which(dist_mahal_rob > umbral_rob)
cat("Número de outliers multivariantes (Mahalanobis robusto):", length(outliers_mahal_rob), "\n")
...
library(DMwR)
k <- 5
dist_knn <- knn.dist(datos_num, k = k)
knn_score <- apply(dist_knn, 1, mean)
umbral_knn <- quantile(knn_score, 0.99)
outliers_knn <- which(knn_score > umbral_knn)
cat("Número de outliers multivariantes (KNN):", length(outliers_knn), "\n")
datos$Outlier_KNN <- FALSE
datos$Outlier_KNN[outliers_knn] <- TRUE
```

## LOF
```{r}
library(DMwR)
lof_score <- lofactor(datos_num, k = k)
umbral_lof <- quantile(lof_score, 0.99)
outliers_lof <- which(lof_score > umbral_lof)
