...
```{r}
datos<-read.csv("C:/Users/Armengol/Downloads/datos_relevantes_Train_Decision3.csv")
datos <- datos %>%
  filter(!is.na(Exited))
hist(datos[, "Exited"])
...
str(datos)
# Prevalencia real del churn (Exited = 1)
p_est <- mean(datos$Exited == 1)
p_est
...
probs_knn <- predict(best_model1, testTransformed, type="prob")[, "1"]

pred_knn_corte <- ifelse(probs_knn > p_corte, "1", "0")
pred_knn_corte <- factor(pred_knn_corte, levels = levels(testTransformed$Exited))

# Calculamos la matriz de confusion 
cm <- confusionMatrix(pred_knn_corte, testTransformed$Exited)
cm
m <- cm$table
...
probs_knn <- predict(best_model2, testTransformed, type="prob")[, "1"]
...
predictionsobs3 <- predict(best_model3, testTransformed,type = "prob")
p_corte <- p_est
p_corte

probs_knn <- predict(best_model3, testTransformed, type="prob")[, "1"]
...
predictionsobs4 <- predict(best_model4, testTransformed,type = "prob")

p_corte <- p_est
p_corte

probs_knn <- predict(best_model4, testTransformed, type="prob")[, "1"]
...
predictionsobs5 <- predict(best_model5, testTransformed,type = "prob")

p_corte <- p_est
p_corte

probs_knn <- predict(best_model5, testTransformed, type="prob")[, "1"]
...
probs_knn_final <- predict(best_model3, newdata = test_final_famd, type = "prob")[, "1"]
pred_knn_final_corte1 <- ifelse(probs_knn_final > p_corte, "1", "0")

pred_test_final_corte1 <- ifelse(pred_knn_final_corte1 == "0", "No", "Yes")

# Crear dataframe con resultados finales
resultado_knn_corte1 <- data.frame(ID = datos_test$ID, Exited = pred_test_final_corte1)
write.csv(resultado_knn_corte1, "resultado_knn_corte1.csv", row.names = FALSE)

```

Al usar el punto de corte vemos como en los otros metodos de balanceo aumenta el f1 score por lo tanto tambien los probaremos para la competencia, como el no balanceado, el undersampling y el ROSE dan los mismos resultados usaremos cualquiera de ellos.
...
probs_knn_final <- predict(best_model4, newdata = test_final_famd, type = "prob")[, "1"]
pred_knn_final_corte2 <- ifelse(probs_knn_final > p_corte, "1", "0")

pred_test_final_corte2 <- ifelse(pred_knn_final_corte2 == "0", "No", "Yes")

# Crear dataframe con resultados finales
resultado_knn_corte2 <- data.frame(ID = datos_test$ID, Exited = pred_test_final_corte2)
write.csv(resultado_knn_corte2, "resultado_knn_corte2.csv", row.names = FALSE)
