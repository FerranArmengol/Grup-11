...
```{r}
datos<-read.csv("datos_relevantes_Train_Decision3.csv")
datos <- datos %>%
  filter(!is.na(Exited))
...
Nos quedamos con los metodos de balanceo sin balanceo, undersampling y ROSE, ya que tienen mayor F1, en este caso la matriz de confusion esta hecha a partir de los valores negativos, haciendo que los KPI's esten al reves, por lo tanto observaremos la especificidad (en el recall).
