...
pred_train <- factor(ifelse(probs_train > 0.345, "1", "0"), levels = levels(train$Exited))
cm_train<- confusionMatrix(pred_train, train$Exited, positive = "1")
cm_train
probs_test <- predict(modelo_smote3, newdata = test, type = "prob")[, "1"]
pred_test <- factor(ifelse(probs_test > 0.345, "1", "0"), levels = levels(test$Exited))
...
tabla_overfitting <- data.frame(
  ...
  Accuracy = c(0.7928, 0.7967),
  Kappa = c(0.4083, 0.4202),
  Recall = c(0.5886, 0.6173),
  Specificity = c(0.8463, 0.8417),
  Precision = c(0.5005, 0.4949)
)
...
pred_submission <- ifelse(probs_submission > 0.345,"Yes", "No")
...
write.csv(submission, "Boosting_XGB_SMOTE_0.345_Decision3.csv", row.names = FALSE)
```