...
probs <- predict(modelo_smote3, newdata = test, type = "prob")[, "Yes"]
...
for(cut in cutoffs){
  ...
  pred <- factor(ifelse(probs > cut, "Yes", "No"), levels = levels(test$Exited))
  cm <- confusionMatrix(pred, test$Exited, positive = "Yes")
