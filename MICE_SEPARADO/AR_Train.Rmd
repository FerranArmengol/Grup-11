...
library(dplyr)
```
Usaremos la base de datos con todas las variables, para poder ver asociaciones entre ellas.
```{r}
bank_data<-read.csv("C:/Users/Armengol/Downloads/datos_missing_train.csv", header = TRUE, stringsAsFactors = TRUE)
...
```
Convertimos todas las variables a fcator ya que las reglas de asociacion solo funcionan con variables categóricas
...
```
Se puede comprobar que mediante reglas redundantes nos salen las mismas que por reglas de apriori.
