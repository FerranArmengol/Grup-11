...
```{r}
data<-read.csv("datos_relevantes_Train_Decision1.csv")
data <- data %>%
  filter(!is.na(Exited))  %>%
  select(-ID)
...
# Balanceo
```{r}
formula_logit <- Exited ~ Age + EstimatedSalary + Gender + Geography + IsActiveMember + Balance + NumOfProducts + CreditScore 
formula_probit <- Exited ~ Age + EstimatedSalary + Gender + Geography + IsActiveMember + Balance + NumOfProducts + CreditScore 
