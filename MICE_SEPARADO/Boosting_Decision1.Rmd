...
#write.csv(submission, "Boosting_GBM_Over_0.545_Decision1.csv", row.names = FALSE)
```
...
probs_xgb <- predict(caret.xgb, test, type="prob")[ , "Class1"]
cutoffs <- seq(0.1, 0.9, by = 0.005)
results <- data.frame(cutoff = numeric(), accuracy=numeric(), recall = numeric(), f1 = numeric())


for (c in cutoffs) {
  pred_corte_test <- factor(ifelse(probs_xgb > c, "Class1", "Class0"), levels = levels(test$Exited))
...
probs <- predict(modelo_smote3, newdata = test, type = "prob")[, "Class1"]
...
datos_test <- datos_test %>% select(-ID)
...
# Predicción con XGB
probs_submission <- predict(caret.xgb, newdata = datos_test, type = "prob")[,"1"]
pred_submission <- ifelse(probs_submission > 0.240,"Yes", "No")
...
#write.csv(submission, "Boosting_XGB_sinBalanceo_0.240_Decision1.csv", row.names = FALSE)
