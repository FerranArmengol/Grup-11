...
# + EVALUACIÓN DE LAS 5 IMPUTACIONES + VISUALIZACIÓN
# ======================================================
...
cat("👉 La mejor imputación combinada (numérica + categórica) es la número:", best_imp, "\n")
...
datos_missing$ID[is.na(datos_missing$ID)] <- sample(ids_disponibles, num_na, replace = FALSE)
...
```{r}
## 1️⃣ Eliminación de variables con baja varianza (Filter)
library(caret)
...
## Análisis de correlación entre variables (Filter)
library(corrplot)
...
# Eliminar variables con correlación > 0.90
high_cor <- findCorrelation(cor_matrix, cutoff = 0.90)
cat("Variables eliminadas por alta correlación:\n")
print(colnames(num_vars)[high_cor])

num_vars_reduced <- num_vars[, -high_cor]

## Selección basada en importancia del modelo (Wrapper / Embedding)
