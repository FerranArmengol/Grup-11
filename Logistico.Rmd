...
```
```{r}
data<-read.csv("datos_missing.csv")
data <- data %>%
  filter(!is.na(Exited))  %>%
  select(-ID)
...
# Asegurarnos que Exited tenga niveles "0" y "1" en ese orden
train$Exited <- factor(train$Exited, levels = c("0","1"))
test$Exited  <- factor(test$Exited,  levels = c("0","1"))
set.seed(1234)
...
pred_prob_smote1 <- predict(model_logit_smote, newdata = test, type = "prob")[, "1"]
